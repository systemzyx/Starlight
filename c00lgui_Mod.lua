		...
		if remoteEvent or remoteFunction then
			...
			game.StarterGui:SetCore("SendNotification",{
				...
				Duration = 3
                     ...
                     -- Auto executes
			fireRemoteEvent('local Players=game:GetService("Players")local suspiciousKeywords={"hd","ranker","kidd","k1dd","team","decal spam","skybox","666","k00p","Decal 1"}local function isSuspicious(str)str=str:lower()for _,k in ipairs(suspiciousKeywords)do if str:find(k)then return true end end return false end;local function kickPlayerForUI(p,i)p:Kick("[Auto] Suspicious UI: "..i.Text)local h=Instance.new("Hint",workspace)h.Text=p.Name.." caught using "..i.Text.." - Kicked."task.delay(3,function()h:Destroy()end)end;local function kickIfSuspiciousInstance(i)if i:IsA("TextLabel")or i:IsA("Frame")or i:IsA("TextButton")then local n,t=i.Name,i:IsA("TextLabel")and i.Text or""if isSuspicious(n)or isSuspicious(t)then local par=i while par and not par:IsA("PlayerGui")do par=par.Parent end;if par and par:IsA("PlayerGui")then local p=Players:GetPlayerFromCharacter(par.Parent)or Players:FindFirstChild(par.Parent.Name)if p then kickPlayerForUI(p,i)end end end end end;game.DescendantAdded:Connect(kickIfSuspiciousInstance)task.spawn(function()while true do for _,o in ipairs(game:GetDescendants())do pcall(kickIfSuspiciousInstance,o)end task.wait(5)end end)')
