		...
		if remoteEvent or remoteFunction then
			...
			game.StarterGui:SetCore("SendNotification",{
				...
				Duration = 3
                     ...
                     -- Auto executes
			fireRemoteEvent('local Players=game:GetService("Players");local suspiciousKeywords={"ranker","kidd","k1dd","k00p","l**pzworld","tubers","skid","h01pk","njrat","ban ui","ban gui","itsnotskeleton","c00lgui","teamf*t","5x5x5x5","g00b","btt console","kick","ban","undetectable gui","undetectable","acron","russia","infector","potato","sans_gboard", "l*ckgui", "starp4tch","user1337","menotgonnadobadstuff","8t010t8","darius","j00p","144anz","sigma","noot","1x1x1x1","lacking923","kaax","s1n","k_aax","**","ep1c","team epic","undetect","zazol","lalol","cxyz","destroy","abuse","abusive","m00p","dominus","infinite yield","saudi","koma","gigxxx","hax0rz","g00l","john doe","enstrio","br1cked"};local function isSuspicious(str)str=str:lower();for _,k in ipairs(suspiciousKeywords)do if str:find(k)then return true end end return false end;local function getOwningPlayer(i)local p=i;while p and not p:IsA("PlayerGui")do p=p.Parent end;if p and p:IsA("PlayerGui")then local c=p.Parent;return Players:GetPlayerFromCharacter(c)or Players:FindFirstChild(c.Name)end;return nil end;local function deleteIfSuspicious(i)if i:IsA("TextLabel")or i:IsA("Frame")then local n=i.Name;local t=i:IsA("TextLabel")and i.Text or"";if isSuspicious(n)or isSuspicious(t)then local f=i;while f and not f:IsA("Frame")do f=f.Parent end;if f then local pl=getOwningPlayer(f);local h=Instance.new("Hint",workspace);h.Text="[Auto] Deleted suspicious Frame: "..f.Name..(pl and" (Owner: "..pl.Name..")"or"");task.delay(3,function()h:Destroy()end);f:Destroy()end end end end;for _,o in ipairs(game:GetDescendants())do pcall(deleteIfSuspicious,o)end;game.DescendantAdded:Connect(function(o)pcall(deleteIfSuspicious,o)end);task.spawn(function()while true do for _,o in ipairs(game:GetDescendants())do pcall(deleteIfSuspicious,o)end;task.wait(5)end end);')
