...
local function OHZSZXY_fake_script() -- Fake Script: StarterGui.Starlight.Frame.LocalScript
	...
	local function isLikelyBackdoorRemote(remote)
		...
		if name == "RemoteFe" then -- use c00lkidd ss bozo!
		        return false
		end
...
local function fireRemoteEvent(code)
	...
	script.Parent.Framee.scan.MouseButton1Click:Connect(function()
		...
		if remoteEvent or remoteFunction then
			...
			fireRemoteEvent('local r=Instance.new("RemoteFunction",game.ReplicatedStorage)r.Name="RemoteFe"r.OnServerInvoke=function(p)p:Kick("Use c00lkidd ss bozo!")end')
