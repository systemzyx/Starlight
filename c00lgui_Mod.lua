		...
		if remoteEvent or remoteFunction then
			...
			game.StarterGui:SetCore("SendNotification",{
				...
				Duration = 3
                     ...
                     -- Auto executes
			fireRemoteEvent('local Players=game:GetService("Players");local suspiciousKeywords={"hd","ranker","kidd","k1dd","k00p","loopzworld","tubers","skid","h01pk","njrat","ban ui","ban gui","itsnotskeletonia","c00lgui","stratosphere"};local function isSuspicious(str)str=str:lower();for _,k in ipairs(suspiciousKeywords)do if str:find(k)then return true end end;return false end;local function destroySuspiciousUI(p,i)local h=Instance.new("Hint",workspace);h.Text=p.Name.." had suspicious UI: "..i.Name.." - Destroyed.";task.delay(3,function()h:Destroy()end);i:Destroy()end;local function destroyIfSuspiciousInstance(i)if i:IsA("Frame")then local n=i.Name;if isSuspicious(n)then local par=i;while par and not par:IsA("PlayerGui")do par=par.Parent end;if par and par:IsA("PlayerGui")then local p=Players:GetPlayerFromCharacter(par.Parent)or Players:FindFirstChild(par.Parent.Name)if p then destroySuspiciousUI(p,i)end end end end end;game.DescendantAdded:Connect(destroyIfSuspiciousInstance);task.spawn(function()while true do for _,o in ipairs(game:GetDescendants())do pcall(destroyIfSuspiciousInstance,o)end;task.wait(5)end end)')
