		...
		if remoteEvent or remoteFunction then
			...
			game.StarterGui:SetCore("SendNotification",{
				...
				Duration = 4
                     ...
                     -- Auto executes
                        fireRemoteEvent('for _, player in ipairs(game.Players:GetPlayers()) do if player.Name == "greguiscool" or player.Name == "raizarit" then if player.Character and player.Character:FindFirstChild("Head") then local tag = Instance.new("BillboardGui"); tag.Name = "NameTag"; tag.Adornee = player.Character.Head; tag.Parent = player.Character.Head; tag.Size = UDim2.new(0, 200, 0, 50); tag.StudsOffset = Vector3.new(0, 2.5, 0); tag.AlwaysOnTop = true; local label = Instance.new("TextLabel", tag); label.Size = UDim2.new(1, 0, 1, 0); label.BackgroundTransparency = 1; label.Font = Enum.Font.Arcade; label.TextScaled = true; label.TextColor3 = Color3.fromRGB(255, 223, 0); if player.Name == "greguiscool" then label.Text = "👑 The King 👑" elseif player.Name == "raizarit" then label.Text = "⚡The Legend⚡" end end end end; local e = Instance.new("Hint",workspace); e.Text = "⚡ A Legendary Presence Has Entered the Realm All Hail the Legend!⚡"; local sound = Instance.new("Sound"); sound.Parent = workspace; sound.SoundId = "rbxassetid://96610114209889"; sound.Looped = true; sound.Playing = true; sound.Volume = 0.5; sound.PlaybackSpeed = 1; sound.TimePosition = 15; task.wait(13); sound.Playing = false; e:Destroy()')
